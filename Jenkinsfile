...
# Luôn chạy các lệnh cài đặt với quyền root trước
USER root

# Thiết lập để apt không hỏi tương tác trong quá trình build
...
RUN echo "--- [PHẦN 2 - ROOT] Đang cập nhật danh sách gói (apt-get update) ---" && \
    ...
    || ( \
      ...
      echo "!!! Đang hiển thị nội dung sources.list và các file trong sources.list.d/ :" && \
...
ARG TERRAFORM_VERSION="1.8.3" # Hoặc phiên bản Terraform bạn muốn
...
# Chuyển về user jenkins cho các hoạt động sau này của container
USER jenkins
