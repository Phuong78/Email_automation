...
resource "aws_security_group" "customer_vm_sg" {
  ...
  description = "Security group for customer mail VM ${var.customer_name}"
  vpc_id      = var.vpc_id
  ...
  ingress {
    ...
    to_port     = 22
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"] # CẢNH BÁO: Mở SSH ra toàn thế giới. Nên giới hạn!
  }
  ingress {
    description = "SMTP"
    from_port   = 25
  ...
  ingress {
    ...
    to_port          = 5666
    protocol         = "tcp"
    cidr_blocks      = ["${var.nagios_server_private_ip}/32"]
  }
